...
- **LimitRange and ResourceQuota** (admission controllers for resource constraints, enforced through the admission flow): covered in `01-pods/assignment-5`
...
Complete `01-pods/assignment-1` for pod spec fundamentals and `12-rbac/assignment-1` for RBAC verification techniques before this topic. Admission controllers are best understood in the context of the full request flow, so the RBAC material (authorization phase) reinforces where admission fits in the sequence.
