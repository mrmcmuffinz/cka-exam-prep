...
Complete `exercises/08-services/` (all assignments) so that Service types, selectors, ports, and endpoints are familiar. Complete `exercises/09-coredns/` for DNS resolution mechanics and the CoreDNS Corefile. Complete `exercises/10-network-policies/` for NetworkPolicy spec structure and default-deny patterns. Complete at least one of the `exercises/11-ingress-and-gateway-api/` assignments for Ingress resource basics. Complete `exercises/19-19-troubleshooting/assignment-1` first; the diagnostic sequence from that assignment (`get pod` then `describe pod` then `logs` then `get events`) is the base pattern extended across the network layer here.
