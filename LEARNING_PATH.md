...
This document provides a structured order for working through the 45 assignments in this repository. The phases are sequenced to build skills progressively, respect topic dependencies, and align with the Mumshad Mannambeth Udemy course structure. Each phase maps to one or more CKA exam domains.
...
**Course alignment:** S6 Cluster Maintenance, S7 Security (TLS sections)
**Prerequisites:** Phase 1 complete
**Estimated time:** 8-10 hours

This phase covers how Kubernetes clusters are built, maintained, and secured at the infrastructure level. You will learn kubeadm workflows, version upgrades, etcd backup/restore, and the certificate infrastructure that secures cluster communication.

| Order | Assignment | Focus |
|---|---|---|
| 8 | cluster-lifecycle/assignment-1 | kubeadm init/join, cluster bootstrapping, node management |
| 9 | cluster-lifecycle/assignment-2 | Version upgrades with kubeadm, drain/cordon workflows |
| 10 | cluster-lifecycle/assignment-3 | etcd backup and restore, disaster recovery |
| 11 | tls-and-certificates/assignment-1 | Kubernetes PKI, certificate creation with openssl |
| 12 | tls-and-certificates/assignment-2 | Certificates API, CertificateSigningRequests |
| 13 | tls-and-certificates/assignment-3 | kubeconfig management, certificate troubleshooting |
...
RBAC controls who can do what in the cluster. This phase covers namespace-scoped permissions (Roles, RoleBindings) and cluster-scoped permissions (ClusterRoles, ClusterRoleBindings), along with service account configuration.

| Order | Assignment | Focus |
|---|---|---|
| 14 | rbac/assignment-1 | Roles, RoleBindings, service accounts, namespace-scoped access |
| 15 | rbac/assignment-2 | ClusterRoles, ClusterRoleBindings, aggregation, cluster-scoped resources |
...
| Order | Assignment | Focus |
|---|---|---|
| 16 | security-contexts/assignment-1 | runAsUser, runAsGroup, fsGroup, runAsNonRoot |
| 17 | security-contexts/assignment-2 | Capabilities (add/drop), allowPrivilegeEscalation |
| 18 | security-contexts/assignment-3 | readOnlyRootFilesystem, seccomp profiles |
| 19 | pod-security/assignment-1 | Pod Security Standards, Pod Security Admission, namespace labels |
...
Beyond Deployments and DaemonSets (covered in Phase 1), Kubernetes has specialized controllers for batch workloads, stateful applications, and automatic scaling. This phase covers those patterns.

| Order | Assignment | Focus |
|---|---|---|
| 20 | jobs-and-cronjobs/assignment-1 | Jobs, CronJobs, completions, parallelism, backoff limits |
| 21 | statefulsets/assignment-1 | StatefulSets, stable identity, ordered deployment, headless services |
| 22 | autoscaling/assignment-1 | HPA, VPA concepts, in-place pod resize |
...
Helm and Kustomize are the two primary tools for managing Kubernetes manifests at scale. This phase covers chart installation and lifecycle management with Helm, and overlay-based customization with Kustomize.

| Order | Assignment | Focus |
|---|---|---|
| 23 | helm/assignment-1 | Chart repositories, helm install, values files |
| 24 | helm/assignment-2 | Upgrades, rollbacks, release history, helm diff |
| 25 | helm/assignment-3 | Chart templates, debugging, creating charts |
| 26 | kustomize/assignment-1 | kustomization.yaml, bases, common transformers |
| 27 | kustomize/assignment-2 | Patches (strategic merge, JSON), patch targets |
| 28 | kustomize/assignment-3 | Overlays, components, multi-environment workflows |
...
| Order | Assignment | Focus |
|---|---|---|
| 29 | crds-and-operators/assignment-1 | CRD authoring, custom resource creation |
| 30 | crds-and-operators/assignment-2 | Working with custom resources, validation, status |
| 31 | crds-and-operators/assignment-3 | Operator pattern, installing and using operators |
| 32 | admission-controllers/assignment-1 | Built-in admission controllers, ValidatingAdmissionPolicy |
...
This phase covers persistent storage in Kubernetes: how volumes are provisioned (statically and dynamically), claimed by pods, and managed over their lifecycle.

| Order | Assignment | Focus |
|---|---|---|
| 33 | storage/assignment-1 | PersistentVolumes, access modes, reclaim policies |
| 34 | storage/assignment-2 | PersistentVolumeClaims, binding, using PVCs in pods |
| 35 | storage/assignment-3 | StorageClasses, dynamic provisioning, volume expansion |
...
**Prerequisites:** Phase 1 complete, Phase 8 recommended
...
| Order | Assignment | Focus |
|---|---|---|
| 36 | services/assignment-1 | ClusterIP, service discovery, endpoints |
| 37 | services/assignment-2 | NodePort, LoadBalancer, external traffic |
| 38 | services/assignment-3 | Headless services, ExternalName, service patterns |
| 39 | coredns/assignment-1 | DNS resolution, service and pod DNS records |
| 40 | coredns/assignment-2 | CoreDNS configuration, Corefile, plugins |
| 41 | coredns/assignment-3 | DNS debugging, resolution failures |
| 42 | network-policies/assignment-1 | Ingress rules, podSelector, namespaceSelector |
| 43 | network-policies/assignment-2 | Egress rules, ipBlock, default deny policies |
| 44 | network-policies/assignment-3 | Policy debugging, traffic flow analysis |
| 45 | ingress-and-gateway-api/assignment-1 | Ingress basics with Traefik controller |
| 46 | ingress-and-gateway-api/assignment-2 | TLS termination with HAProxy Ingress |
| 47 | ingress-and-gateway-api/assignment-3 | Gateway API with Envoy Gateway |
| 48 | ingress-and-gateway-api/assignment-4 | Gateway API with NGINX Gateway Fabric |
| 49 | ingress-and-gateway-api/assignment-5 | Ingress to Gateway API migration (Ingress2Gateway) |
...
|---|---|---|---|
| Cluster Architecture, Installation & Configuration | 25% | 2, 3, 6, 7 | 17 |
| Workloads & Scheduling | 15% | 1, 4, 5 | 14 |
| Services & Networking | 20% | 9 | 14 |
| Storage | 10% | 8 | 3 |
...
The phase order above is the recommended path for learners following the Mumshad course in sequence. However, some phases are independent and can be reordered based on your priorities:

**If you want to focus on exam weight:** After Phase 1, jump to Phase 9 (Networking, 20%) and Phase 10 (Troubleshooting, 30%) since these two domains account for half the exam.
...
**If you need RBAC immediately:** Phase 3 only requires Phase 1 as a prerequisite. You can complete it before Phase 2.

**If you are weak on storage:** Phase 8 only requires Phase 1. You can prioritize it earlier if storage is a gap area.
