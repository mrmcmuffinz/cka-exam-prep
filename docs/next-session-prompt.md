...
- Phase 4 is in progress. **2 of approximately 19 assignments content-complete:**
...
1. **Pick the next Phase 4 task from the priority queue in `docs/session-handoff.md`.** Top choices: `rbac/assignment-2`, `troubleshooting/assignment-2`, `troubleshooting/assignment-4` (stub-to-full regenerations, highest visual impact per assignment), or `statefulsets/assignment-1`, `autoscaling/assignment-1`, `admission-controllers/assignment-1` (new content closing remaining curriculum gaps). Tell me which one you plan to generate and confirm you have read the four essential documents before writing anything.
